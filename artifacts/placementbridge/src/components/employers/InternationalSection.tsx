...
export function InternationalSection() {
  return (
    <section className="py-24 employer-gradient relative overflow-hidden">
      ...
      <div className="absolute top-20 left-20 w-80 h-80 bg-blue-500lue-500/5 rounded-full blur-[120px]" />
          ...
          {routes.map((route, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <ArrowRight className="h-4 w-4 text-blue-400lue-400" />
                <div className="flex items-center gap-2">
              ...
              {route.active ? (
                ...
                <Badge className="bg-white/5 text-blue-400lue-300/30 border-white/10 rounded-full text-xs">
                  Coming Soon
                </Badge>
              )}
            </motion.div>
        ...
        >
          <div className="px-8 py-6 border-b border-white/5">
            <h3 className="text-xl font-bold text-white flex items-center gap-2">
              <Plane className="h-5 w-5 text-blue-400lue-400" />
            ...
            {workflowSteps.map((step, i) => (
              ...
              >
                ...
                <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-blue-600/20 to-indigo-600/20 border border-blue-500/20 flex items-center justify-center mx-auto mb-3 group-hover:scale-110 transition-transform">
                  <step.icon className="h-7 w-7 text-blue-400lue-400" />
                </div>
                <h4 className="text-white font-bold text-sm mb-1">{step.title}</h4>
                <p className="text-blue-400lue-300/50 text-xs">{step.description}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        >
          {[
            ...
            <div key={stat.label} className="text-center bg-white/5 rounded-2xl p-5 border border-white/5">
              ...
              <div className="text-sm text-blue-400lue-300/60 mt-1">{stat.label}</div>
            </div>
          ))}
        </motion.div>
